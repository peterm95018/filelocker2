...
def requires_login(permissionId=None, **kwargs):
    ...
    if cherrypy.session.has_key("user") and cherrypy.session.get('user') is not None:
        ...
        try:
            ...
            # copied CAS block above and adjusted for Shibboleth 
            if authType == "saml":
                shibboleth_logout_url = session.query(ConfigParameter).filter(ConfigParameter.name=="shibboleth_logout_ur
l").one().value
                #if cherrypy.request.params.has_key("ticket"):
                if authType == "saml":
                    #valid_ticket, userId = casConnector.validate_ticket(rootURL, cherrypy.request.params['ticket'])
                    userId = cherrypy.request.headers['X-UID']
                    #if valid_ticket: 
                    # If we made it here from Shibboleth, we have a valid authentication session
                    if authType == "saml":
                        currentUser = AccountService.get_user(userId, True)
                        cherrypy.session['request-origin'] = str(os.urandom(32).encode('hex'))[0:32]
                        if currentUser is None:
                            # user does not exist yet in FL MySQL DB
